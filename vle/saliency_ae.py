...
class AutoEncoder(pl.LightningModule):
    def __init__(
        ...
        self.saliency = instantiate_from_config(saliency)
    ...
    def forward(self, x, n_tokens, compute_losses=False, log=False):
        ...
        for i in range(n_tokens):
            # Saliency model output
            residuals, mean, std = normalize(x - rec)
            model_output = self.saliency(residuals)
            i_rec, i_mask = torch.split(model_output, (3, 1), dim=1)
            i_mask = torch.sigmoid(i_mask)

            # AE model output
            ae_input = torch.cat([i_rec, input_norm(i_mask)], dim=1)
            i_rec = self.decode(self.encode(ae_input))
            i_rec = i_rec * std + mean              

            # Compute intermediate mask & reconstruction losses
            if compute_losses:
    ...
    def set_n_tokens(self, n_tokens):
        ...
        if hasattr(self.saliency, "set_n_tokens"):
            self.saliency.set_n_tokens(n_tokens)
    ...
    def reset_hidden_state(self):
        ...
        if hasattr(self.saliency, "reset_hidden_state"):
            self.saliency.reset_hidden_state()
